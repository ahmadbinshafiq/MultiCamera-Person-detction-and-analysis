...
PARENT_ROOT = FILE.parents[1]  # ML-pipeline root directory
ROOT = FILE.parents[0]
WEIGHTS = PARENT_ROOT / 'weights'

if str(ROOT) not in sys.path:
    sys.path.append(str(ROOT))  # add ROOT to PATH
if str(ROOT / 'trackers' / 'strong_sort') not in sys.path:
    sys.path.append(str(ROOT / 'trackers' / 'strong_sort'))  # add strong_sort ROOT to PATH
if str(PARENT_ROOT / 'yolov5_tracker' / 'trackers' / 'strong_sort') not in sys.path:
    sys.path.append(str(PARENT_ROOT / 'yolov5_tracker' / 'trackers' / 'strong_sort'))  # add strong_sort ROOT to PATH

PARENT_ROOT = Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
def load_detector(weights=WEIGHTS / 'yolov5s.pt', device='', imgsz=640, dnn=False, half=False):
    ...
    print("Loaded detector model from: ", weights)
...
def load_tracker(nr_sources=1,
    ...
    print("Loaded tracker model from: ", reid_weights)
